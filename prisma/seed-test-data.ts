...
async function main() {
  ...
  for (const user of testUsers) {
    await prisma.notificationPreferences.upsert({
      ...
      create: {
        ...
        usageAlerts: Math.random() > 0.3,
        systemAlerts: Math.random() > 0.1,
        teamAlerts: Math.random() > 0.5,
        reports: Math.random() > 0.3,
        recommendations: Math.random() > 0.4
      }
    })
  }
  ...
  // Instructions are stored for reference but not in database
  // as UserSettings doesn't have a metadata field
  console.log('✅ Created predefined instructions (in memory)')
